...
def createRepo():
    if request.method == "POST":
        ...
        try:
            ...
            conn.execute(
                "INSERT INTO repos (name, owner_id) VALUES (?, ?)", (name, user_id)
            )
            conn.commit()
...
def userSettings():
    if request.method == "POST":
        ...
        conn = database.getDb()
        conn.execute(
            "UPDATE users SET git_password_hash = ?, git_password_salt = ? WHERE id = ?",
    ...
    conn = database.getDb()
    repos = conn.execute(
        "SELECT * FROM repos WHERE owner_id = ?", (session["user_id"],)
    ).fetchall()
    conn.close()
...
def viewRepo(repo_name):
    ...
    if git_utils.isRepoEmpty(repo_name):
        clone_url = url_for(
            "gitSmartHttp", repo_name=repo_name, subpath="", _external=True
        ).rstrip("/")
...
def viewTree(repo_name, ref_path):
            ...
            try:
                ...
                if file["name"].lower().endswith(".md"):
                    readme_html = markdown.markdown(
                        text_content, extensions=["fenced_code", "codehilite", "tables"]
                    )
                else:
    ...
    parent_ref_path = None
    if path:
        parent_path = "/".join(path.split("/")[:-1])
        parent_ref_path = ref if not parent_path else ref + "/" + parent_path
...
def viewBlob(repo_name, ref_path):
    ...
    try:
        ref, path = git_utils.splitRefPath(repo_name, ref_path)
        content_bytes = git_utils.runGit(
            repo_name, ["cat-file", "blob", f"{ref}:{path}"], encoding=None
        )
...
def viewCommits(repo_name, ref):
    ...
    return render_template(
        "commits.html", repo_name=repo_name, ref=ref, commits=commits
    )
...
def verifyBasicAuth(auth_header):
    ...
    try:
        ...
        conn = database.getDb()
        user = conn.execute(
            "SELECT * FROM users WHERE preferred_username = ?", (username,)
        ).fetchone()
        conn.close()
...
def gitSmartHttp(repo_name, subpath):
    ...
    if is_write:
        ...
        if not user:
            return Response(
                "Unauthorized", 401, {"WWW-Authenticate": f'Basic realm="{repo_name}"'}
            )
...
def importRepoCli(repo_name, owner_username):
    ...
    try:
        ...
        # 3. Check Owner
        user = conn.execute(
            "SELECT id FROM users WHERE preferred_username = ?", (owner_username,)
        ).fetchone()
        if not user:
            print(
                f"Error: User '{owner_username}' not found. Please login via Web UI first to create the user."
            )
            return
        ...
        # 4. Insert
        conn.execute(
            "INSERT INTO repos (name, owner_id) VALUES (?, ?)", (repo_name, owner_id)
        )
        conn.commit()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--import-repo",
        help="Import an existing repository by name (folder name without .git)",
    )
    parser.add_argument("--owner", help="Owner username for the imported repo")
    parser.add_argument(
        "--config", default="baregit.ini", help="Path to configuration file"
    )
